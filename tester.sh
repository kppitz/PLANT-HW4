...
inputs=`ls hw4_input --ignore-backups`
echo "--> Running against test cases"
for i in $inputs; do
  echo "--> Trying" $i
  ./a.out < ./hw4_input/$i > ./hw4_output/$i.out
 
  diff ./hw4_output/$i.out ./hw4_expected_output/$i.out > \
       ./reports4/$i --ignore-space-change --ignore-case \
       --side-by-side
 
  lines=`diff ./hw4_output/$i.out ./hw4_expected_output/$i.out \
         --ignore-space-change --ignore-case  | wc --lines`
  echo $lines unexpected lines in $i.out \
       >> ./reports4/summary.txt
  echo $lines errors
 done
